...
def find_img(i, h, w, depth, rd, gr, bl, orient, color_ord, folder):
            ...
            for c in range(width):
                if len(bits) < bits_needed:
                    if len(bits) == bits_needed:
                        break
                    if red:
                        bits += (bin((img[r,c,0] & lsbs))[2:].zfill(int(depth)))
                    if len(bits) == bits_needed:
                        break
                    if green:
                        bits+=(bin((img[r,c,1] & lsbs))[2:].zfill(int(depth)))
                    if len(bits) == bits_needed:
                        break
                    if blue:
                        bits+=(bin((img[r,c,2] & lsbs))[2:].zfill(int(depth)))
    ...
    imageio.imwrite(folder+"/"+i[7:], image)
if __name__ == "__main__":
    find_img(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], sys.argv[5], sys.argv[6], sys.argv[7], sys.argv[8], sys.argv[9], sys.argv[10])
